...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='ResNet50_CIFAR100_32_BM_noremap')
# reproducibility
...
if len(trainset_CIFAR100) <= len(trainset_attack):
...
for ep in range(start_epoch, args.epochs):
    ...
    print("CIFAR100:")
    acc_C = evaluator(model, testset_CIFAR100_loader, criterion, device)
    print("SVHN:")
    ...
    if acc_C > best_acc:
        print('Best accuracy for Cifar100 updated from %.3f%% to %.3f%%.' % (best_acc, acc_C))
        best_acc = acc_C
        print('Saving...')
        state = {
