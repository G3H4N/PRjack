...
transfer_set = 'Enc_GTSRB_prune_ft1-7_all7_Chameleon%d_Enc%d%d-64'%(alpha, enc_num_residual_blocks, enc_num_updownsampling)
...
print('Attack success rate of the target model with the transformer is:')
validate_hijack_transfer(target, Enc, testset_attack_loader, criterion, device)

print("Finished training!")

for pruning_rate in [0.3, 0.5, 0.7, 0.9]:
    print("======== Sparsity = %f ========"%(pruning_rate))
    ...
    for set_pruner in ['taylorfo']:#, 'agp', 'lottery', 'AutoCompress', 'AMC']:
        ...
        with suppress_stdout_stderr():
            try:
                shadow, masks = prune_model_wo_finetune(shadow, input_size, criterion, set_pruner, args.pruning_algo, pruning_rate,
        ...
        save_checkpoint(state, '%s/Epoch_(%d).ckpt' % (pruned_target_ckpt_dir, ep + 1),
                        max_keep=2, is_best=False)
        '''
        print("Pruner: ", set_pruner, ", Sparsity: ", pruning_rate)
